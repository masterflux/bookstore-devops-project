...
export default function CartPage() {
    ...
    useEffect(() => {
        if (!isLoggedIn) {
            ...
            fetch((window.origin + '/api') + '/cart/' + sessionStorage.getItem('token'), {
                method: 'GET',
            }).then((res) => res.json())
                .then((data) => {
                    setCart(data)
    ...
    const updateQuantity = async (id: number, change: number) => {
        ...
        await fetch((window.origin + '/api') + `/cart/${updatedCart.find((item) => item.id === id)?.bookId}`, {
            method: 'PUT',
    ...
    const clearCart = async () => {
        await fetch((window.origin + '/api') + '/cart/clear/' + sessionStorage.getItem('token'), { method: 'DELETE' })
        setCart([])
        setTotalAmount(0)
        setQuantity(0)
    }
